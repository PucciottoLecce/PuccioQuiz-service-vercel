...
async function sendWinnerEmail({ to, prizeName, token, prizeValidUntil }) {
  ...
  const subject = "Il tuo omaggio — PuccioQuiz 🎉";
  const text =
    `Complimenti! Il tuo omaggio è: ${prizeName}\n\n` +
    `Il tuo token da riscattare è: ${token}\n` +
    (untilIt ? `Valido fino al: ${untilIt}\n` : "") +
    `\nMostra questo token in negozio per riscattare il tuo omaggio.`;
...
<style>
  body { margin:0; padding:0; background:#fafaf8; font-family:'Segoe UI',Arial,sans-serif; color:#1a1a1a; }
  ...
  .header h1 { margin:0; font-family:Georgia,serif; font-size:32px; color:#fff; letter-spacing:1px; }
  .header p  { margin:6px 0 0; color:rgba(255,255,255,.8); font-size:13px; }
  .body { padding:28px 32px; }
  .token-box { background:#fafaf8; border:2px dashed #e8e4e0; border-radius:12px; padding:16px 20px; text-align:center; margin:20px 0; }
  .token-label { font-size:11px; color:#999; letter-spacing:2px; text-transform:uppercase; margin-bottom:6px; }
  .token-value { font-size:26px; font-weight:700; color:#C0392B; letter-spacing:3px; }
  .prize-box { background:#fff8f4; border:1px solid #fde8e2; border-radius:12px; padding:20px 24px; text-align:center; margin:20px 0; }
  .prize-emoji { font-size:48px; display:block; margin-bottom:8px; }
  .prize-label { font-size:22px; font-weight:700; color:#C0392B; margin-bottom:4px; }
  .prize-name  { font-size:18px; font-weight:700; color:#1a1a1a; }
  .footer { background:#fafaf8; padding:20px 32px; text-align:center; font-size:12px; color:#aaa; border-top:1px solid #f0ece8; }
</style>
</head>
<body>
<div class="wrap">
  <div class="header">
    <h1>Pucciotto</h1>
    <p>La puccia 2.0 — Lecce</p>
  </div>
  <div class="body">
    <p style="font-size:16px;margin:0 0 8px;">Complimenti! 🎉</p>
    <p style="font-size:14px;color:#555;margin:0 0 20px;">
      Hai completato il PuccioQuiz e hai vinto un omaggio. Mostra il token qui sotto in negozio per ritirarlo.
    </p>

    <div class="prize-box">
      <span class="prize-emoji">🎁</span>
      <div class="prize-label">Il tuo omaggio</div>
      <div class="prize-name">${escapeHtml(prizeName)}</div>
    </div>

    <div class="token-box">
      <div class="token-label">Token da mostrare in cassa</div>
      <div class="token-value">${escapeHtml(token)}</div>
      ${untilIt ? `<div style="font-size:12px;color:#aaa;margin-top:8px;">Valido fino al: <strong>${escapeHtml(untilIt)}</strong></div>` : ""}
    </div>

    <p style="font-size:13px;color:#777;margin:0;">
      📍 Vieni a trovarci in <strong>Via dei Biccari 15, Lecce</strong> e mostra questo token alla cassa per ritirare il tuo omaggio.
    </p>
  </div>
  <div class="footer">
    Pucciotto Srls · Via dei Biccari 15, 73100 Lecce · P.IVA 05465530755<br/>
    <a href="mailto:info@pucciotto.it" style="color:#C0392B;">info@pucciotto.it</a>
  </div>
</div>
</body>
</html>`;
